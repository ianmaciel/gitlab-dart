  ...
  group('GitLab', () {
    ...
    group('.buildUri()', () {
      test('adds pagination info when necessary', () {
        ...
        expect(uri.toString(), 'https://gitlab.com/api/v4/foo?page=3&per_page=10');
      });
      test('adds queryParameters', () {
        Uri uri;

        uri = gitLab.buildUri(['foo'], queryParameters: {'foobar': 'test'}, page: 6);
        expect(uri.toString(), 'https://gitlab.com/api/v4/foo?foobar=test&page=6');
        ...
        expect(uri.toString(), 'https://gitlab.com/api/v4/foo?foo=test&bar%5B%5D=a&bar%5B%5D=b&bar%5B%5D=c');
      });
